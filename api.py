...
from core.param_stats import record_param_stats as _record_param_stats_core
from core.query_history import record_query, get_history
from core.connection_pool import ConnectionPool, ConnectionPoolExhausted
...
def _on_startup() -> None:
    ...
    if COGNITIVE_GOAL_ENABLED and _guardian_graph is not None:
        try:
            from core.cognitive_goal import CognitiveGoalManager
...
def _on_shutdown() -> None:
    """应用关闭时清理连接池和专家管理器"""
    global _pool, _expert_manager, _checkpoint_manager, _checkpoint_graph, _guardian_loop, _guardian_graph, _goal_mgr, _curiosity_engine, _perception_manager, _perception_graph
...
async def get_aliases():
    ...
    try:
        graph = _pool.acquire()
        from core.alias_expander import AliasExpander
...
async def get_candidate_seeds():
    ...
    try:
        graph = _pool.acquire()
        from core.seed_candidate import SeedCandidateManager
...
def list_meta_seeds(category: str | None = None):
    ...
    try:
        graph = _pool.acquire()
        from core.meta_seed import MetaSeedManager, MetaSeedCategory
...
def get_meta_seed(label: str):
    ...
    try:
        graph = _pool.acquire()
        from core.meta_seed import MetaSeedManager
        mgr = MetaSeedManager(graph)
...
def _record_param_stats(
    ...
    委托给 core.param_stats.record_param_stats 实现。
    记录失败不影响查询结果返回。
...
def list_cognitive_goals(
    ...
    try:
        graph = _pool.acquire()
        from core.cognitive_goal import CognitiveGoalManager, GoalStatus, GoalType as GT
...
def create_cognitive_goal(body: CreateCognitiveGoalRequest):
    ...
    from core.cognitive_goal import GoalType
